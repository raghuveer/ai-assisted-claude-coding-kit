...
# is not idempotence, it is data loss. Git runs only ONE commit-msg hook, so the old one
# stops firing either way; back it up and make that the operator's decision, not a silent
# side effect of adopting the kit.
if [ -e "$HOOK" ] && ! grep -q 'GENERATED by kit-init.sh' "$HOOK" 2>/dev/null; then
  BAK="$HOOK.pre-kit"
  if [ -e "$BAK" ]; then BAK="$HOOK.pre-kit.$(date -u +%Y%m%d%H%M%S)"; fi
  cp "$HOOK" "$BAK"
  echo "WARNING: existing commit-msg hook saved as ${BAK##*/}"
...
ROOT_P=$(cd "$ROOT" && pwd)
echo "generated ${HOOK#$ROOT_P/} (tooling: $HERE)"
