...
the operator. 6. **Findings** — returned as DATA, not prose: your whole reply is ONE JSON object `{"verdict": ..., "narrative": "<everything above, as markdown>", "findings": [ ... ]}`, piped into `kit-finding.sh --json`. One finding object per surviving mutant and design concern, each REQUIRING `class`, `severity` and a one-line `summary` naming it (**at most 200 characters** — the explanation goes in `narrative`, and an over-long one rejects the WHOLE batch), plus optional `lang`, `pattern`, `domain`, `file` and `line`. Nothing should have to parse you — a person retyping a prose block dropped the `pattern` on every finding of a review on 2026-08-10. Found nothing? `"findings": []`, which is a measurement; omitting the key is not. `class` is one of: fail-open race false-rationale perf compliance correctness style unclassified; `severity` is one of: critical major minor nit. Use `unclassified` rather than inventing a name -- an unrecognised value is rejected, and one bad value rejects the WHOLE batch. Asserted against `kit-finding.sh --vocab` by tests/conformance.sh.
