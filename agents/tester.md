...
the operator. 6. **Findings (recordable)** — one per line, `class|severity|lang|domain`, for every surviving mutant and design concern; these pipe into `kit-finding.sh --batch`. Run `kit-finding.sh --vocab` for the accepted values; an unrecognised one is rejected, not stored.
